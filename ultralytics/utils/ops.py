...
def non_max_suppression(
    ...
    if isinstance(prediction, dict):  # YOLOv10 model output format
        prediction = prediction["one2one"]  # select one2one output
    
    if isinstance(prediction, (list, tuple)):  # Handle tuple/list from one2one
        prediction = prediction[0]  # select first element
